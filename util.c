...
int is_powerof_two(int x) { return ((x != 0) && !(x & (x - 1))); }
...
int kth_bit(int n, int k) { return (n >> k) & 1; }
...
int *mapping_coreid() {
  ...
  fp = popen("awk -F ': ' '/core id/ {print $2}' /proc/cpuinfo | paste -sd ' '",
             "r");
...
int *mapping_apicid() {
  ...
  // each "processor" (ie processor 0, then 1, etc)
  fp = popen(
      "awk -F ': ' '/initial apicid/ {print $2}' /proc/cpuinfo | paste -sd ' '",
      "r");
